package com.app.server.businessservice.organizationextendedbc.locationextended.aspect;
...
@Aspect
@Component
public class BusinessServicelocationextendedAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.app.server.businessservice.organizationextendedbc.locationextended..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("OEBLE217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("OEBLE227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            AppAlarm appAlarm = Log.getExceptionAlarm("OEB", "LE", e.getClass().getName());
